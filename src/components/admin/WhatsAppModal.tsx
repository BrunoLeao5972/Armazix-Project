...
export function WhatsAppModal({ open, onClose }: WhatsAppModalProps) {
  return (
    <Dialog open={open} onOpenChange={v => { if (!v) onClose(); }}>
      <DialogContent className="sm:max-w-[620px] rounded-2xl p-0 overflow-hidden gap-0 max-h-[92vh] flex flex-col">
        ...
        <DialogHeader className="px-6 pt-6 pb-4 border-b border-border/50 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle className="text-base font-semibold leading-tight">Configurações WhatsApp</DialogTitle>
              <p className="text-xs text-muted-foreground mt-0.5">Conexão, notificações e templates de mensagem</p>
            </div>
          </div>
        </DialogHeader>

        <div className="px-6 py-6 overflow-y-auto flex-1 min-h-0">
          {open && <WhatsAppConfigTab />}
        </div>
