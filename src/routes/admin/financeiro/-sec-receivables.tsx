...
// DENTRO do corpo de ModalEditarContaReceber/ModalNovaContaReceber — cada
        ...
        <div className="p-5 space-y-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              ...
              <EditarContaReadOnly label="Responsável" value={conta.responsavel} />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <EditarContaField label="Acréscimo" value={form.juros} onChange={v => set("juros", v)} disabled={regra.bloquearValores} placeholder="0,00" />
      ...
      <div className="relative bg-card border border-border/50 rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-0">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <NovaContaField label="Cliente *" value={f.cliente} onChange={v => set("cliente", v)} placeholder="Nome do cliente" />
            <NovaContaField label="Documento" value={f.documento} onChange={v => set("documento", v)} placeholder="Ex: VD-1099" />
            <NovaContaField label="Descrição *" value={f.desc} onChange={v => set("desc", v)} placeholder="Ex: Venda #1099" />
            <NovaContaSelect label="Histórico" value={f.categoria} onChange={v => set("categoria", v)} opts={HISTORICOS.filter(h => h.natureza === "RECEITA" && h.nivel === 3).map(h => historicoLabel(h))} />
            <NovaContaSelect label="Centro de Custo" value={f.centroCusto} onChange={v => set("centroCusto", v)} opts={["Loja","Admin","Operacional"]} />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <NovaContaSelect label="Forma de Pagamento" value={f.formaPgto} onChange={v => set("formaPgto", v)} opts={["PIX","Dinheiro","Cartao","Boleto","Transferencia"]} />
            <NovaContaSelect label="Conta Financeira" value={f.contaFinanceira} onChange={v => set("contaFinanceira", v)} opts={["Caixa","Banco","Carteira"]} />
